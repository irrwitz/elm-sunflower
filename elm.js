...
Elm.Main.make = function (_elm) {
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var phi = ($Basics.sqrt(5) + 1) / 2;
   var max_d = 300;
   var tau = $Basics.pi * 2;
   var scale_factor = 4;
   var seed_radius = 2;
   var drawSeed = F2(function (x,y) {
      return A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: x,_1: y},A2($Graphics$Collage.filled,$Color.orange,$Graphics$Collage.circle(seed_radius)));
   });
   var draw = function (i) {
      var r = $Basics.sqrt(i) * scale_factor;
      var theta = i * tau / phi;
      var x = r * $Basics.cos(theta);
      var y = r * $Basics.sin(theta);
      return A2(drawSeed,x,y);
   };
   ...
   var view = function (seed) {
      var canvas = $Html.fromElement(A3($Graphics$Collage.collage,300,300,drawAll($String.toFloat(seed))));
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: "300px"},{ctor: "_Tuple2",_0: "margin",_1: "0 auto"}]))]),
      _U.list([A2($Html.h2,_U.list([]),_U.list([$Html.text("Dr. Fibonacci\'s Sunflower Spectacular")]))
              ,canvas
              ,A2($Html.div,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "text-align",_1: "center"}]))]),
              _U.list([A2($Html.label,_U.list([]),_U.list([$Html.text("1")]))
                      ,A2($Html.input,
                      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "vertical-align",_1: "middle"}]))
                              ...
                              ,A3($Html$Events.on,"change",$Html$Events.targetValue,$Signal.message(mbox.address))]),
                      _U.list([]))
                      ,A2($Html.label,_U.list([]),_U.list([$Html.text(" 1000")]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "text-align",_1: "center"}
                                                      ,{ctor: "_Tuple2",_0: "padding-top",_1: "8px"}
                                                      ,{ctor: "_Tuple2",_0: "display",_1: "block"}]))]),
              _U.list([A2($Html.span,_U.list([]),_U.list([$Html.text(seed)]))]))]));
   };
