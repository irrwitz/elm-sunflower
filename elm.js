...
Elm.Main.make = function (_elm) {
   ...
   var drawAll = function (seed) {
      var _p0 = seed;
      if (_p0.ctor === "Ok") {
            return A2($List.map,draw,_U.range(0,_p0._0));
         } else {
            return A2($List.map,draw,_U.range(0,1000));
         }
   };
   ...
   var view = function (i) {
      ...
      return A2($Html.div,
      _U.list([]),
      _U.list([$Html.fromElement(A3($Graphics$Collage.collage,600,600,drawAll($String.toFloat(i))))
              ,A2($Html.label,_U.list([]),_U.list([$Html.text("0")]))
              ,A2($Html.input,
              _U.list([$Html$Attributes.type$("range"),$Html$Attributes.min("1"),$Html$Attributes.max("1000"),$Html$Attributes.value(i),evth]),
              _U.list([]))
              ,A2($Html.label,_U.list([]),_U.list([$Html.text("1000")]))
              ,A2($Html.input,_U.list([$Html$Attributes.type$("text"),$Html$Attributes.value(i),evth]),_U.list([]))]));
   };
